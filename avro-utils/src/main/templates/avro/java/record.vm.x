...
import com.mazeboard.avro.AsJava;
import com.mazeboard.avro.AsScala;
...
public class ${this.mangle($schema.getName())}#if ($schema.isError()) extends org.apache.avro.specific.SpecificExceptionBase#else extends org.apache.avro.specific.SpecificRecordBase#end implements org.apache.avro.specific.SpecificRecord, DefinedByConstructorParams {
...
#foreach ($field in $schema.getFields())
  ...
  public ${this.mangle($schema.getName())}(
  #foreach($field in $schema.getFields())
     #if(${this.javaUnbox($field.schema()).indexOf("java.util.List<")} == 0) scala.collection.immutable.List${this.javaUnbox($field.schema()).substring(14,${this.javaUnbox($field.schema()).length()})}
     #elseif(${this.javaUnbox($field.schema()).indexOf("java.util.Map<")} == 0) scala.collection.immutable.Map${this.javaUnbox($field.schema()).substring(13,${this.javaUnbox($field.schema()).length()})}
...
#foreach ($field in $schema.getFields())
   ...
   * objects instead.
   *
   * this schema cannot have a spark encoder
   */
#end
#end

  // the constructor without arguments must be declared after the all-args constructor
  // so that the scala reflection selects the all-args constructor as the primary constructor
  public ${this.mangle($schema.getName())}() {}
